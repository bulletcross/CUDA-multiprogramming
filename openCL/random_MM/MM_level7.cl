__kernel void MM_level7(__global const int4* A, __global const int4* B, __global int* C){
  ...
  for(int tile = 0; tile<32; tile++){
    for(int copy_load = 0; copy_load<(16/4); copy_load++){
      ...
      B_tile_temp[y_index*4+0][x_index] = vec_b.x;
      B_tile_temp[y_index*4+1][x_index] = vec_b.y;
      B_tile_temp[y_index*4+2][x_index] = vec_b.z;
      B_tile_temp[y_index*4+3][x_index] = vec_b.w;
    }
    barrier(CLK_LOCAL_MEM_FENCE);

    for(int k = 0; k<32; k++){
      for(int i=0;i<8;i++){
        B_cached[i] = B_tile_temp[k][16*i + y_local];
      }
